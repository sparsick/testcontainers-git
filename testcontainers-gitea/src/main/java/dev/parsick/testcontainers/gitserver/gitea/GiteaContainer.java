package dev.parsick.testcontainers.gitserver.gitea;
...
/** Container for a Gitea Git server based on the Docker image "gitea/gitea". */
public class GiteaContainer extends GenericContainer<GiteaContainer> {

  private static DockerImageName DEFAULT_DOCKER_IMAGE_NAME = DockerImageName.parse("gitea/gitea");
  ...
  /**
   * Creates a new {@code giteaContainer} with the given Docker image.
   *
   * @param dockerImageName the Docker image to use; must be compatible with {@code gitea/gitea}
   */
  public GiteaContainer(DockerImageName dockerImageName) {
   ...
   * @return instance of the gitea container
   */
  public GiteaContainer withInitUserName(String initUserName) {
   ...
   * @return instance of the gitea container
   */
  public GiteaContainer withInitUserPassword(String initUserPassword) {
   ...
   */
  public GiteaContainer withGitRepo(String gitRepoName) {
   ...
   * @return instance of the git server container
   */
  public GiteaContainer withSshKeyAuth() {
    try {
      sshClientIdentity =
          new SshIdentity(
   ...
   * @return instance of the gitea container
   */
  public GiteaContainer withCopyExistingGitRepoToContainer(String pathToExistingRepo) {
  ...
  private void configureAdminUser() {
    try {
      String command =
          String.format(
              "gitea admin user create --username %s --password %s --email admin@example.com --admin",
